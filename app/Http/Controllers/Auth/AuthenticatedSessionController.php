...
class AuthenticatedSessionController extends Controller
{
    ...
    public function store(LoginRequest $request)
    {
        ...
        {
            auth()->logout();
            return redirect()->route('login')->with('error', __('Your account has been deleted.'));
        }

        if($user->is_active == 0)
        {
            auth()->logout();
            return redirect()->route('login')->with('error', __('Your account is inactive.'));
        }
        
        // Re-fetch user after potential logout (shouldn't be needed, but safety check)
        $user = \Auth::user();
        if(!$user)
        {
        ...
        if($user->type == 'company')
        {
            ...
            if($plan)
            {
                if($plan->duration != 'lifetime' && !empty($user->plan_expire_date))
                {
                    ...
                    if($days <= 0)
                    {
                        ...
                        return redirect()->intended($this->resolveDashboardRedirectPath($user))->with('error', __('Your Plan is expired.'));
                    }
                }
            }
...
//            $ip = '49.36.83.154'; // This is static ip address
        ...
        // Super admin goes to main dashboard, all others go to account dashboard
        if(!$user)
        {
            return redirect()->route('login')->with('error', __('User session expired.'));
        }
        
        $redirectPath = $this->resolveDashboardRedirectPath($user);
     ...
     * @return string
     */
    protected function resolveDashboardRedirectPath($user)
    {
        if(!$user)
        {
            return route('login');
        ...
        {
            return route('client.dashboard.view');
        }
        ...
        if($user->type == 'company')
        {
            return url('/account-dashboard');
        }

        $firstRoute = $user->getFirstAccessibleRoute();
        if(!empty($firstRoute))
        {
            return $firstRoute;
        }
