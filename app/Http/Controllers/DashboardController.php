...
class DashboardController extends Controller
{
    ...
    public function account_dashboard_index()
    {
        if(Auth::check())
        {
            ...
            {
                if(request()->is('dashboard') && Auth::user()->type != 'super admin')
                {
                    return redirect('account-dashboard');
                }
                // For company and other users, check permission
                // Company users should have this permission via their role, but allow them access anyway
                $hasPermission = \Auth::user()->can('show account dashboard');
                $isCompany = \Auth::user()->type == 'company';
                
                if($hasPermission || $isCompany)
                {
                    try {
                    ...
                    try {
                        ...
                        $data['incExpBarChartData'] = ['income' => [], 'expense' => [], 'month' => []];
                    }
                    ...
                    if($data['users']) {
                        $data['plan']  = Plan::find($data['users']->plan ?? 1);
                        ...
                        $data['plan'] = Plan::find(1);
                        $data['storage_limit'] = 0;
                    }

                    // Ensure all required variables are set
                    if(!isset($data['incExpLineChartData'])) {
                        $data['incExpLineChartData'] = ['income' => [], 'expense' => [], 'day' => []];
                    }
                    if(!isset($data['incExpBarChartData'])) {
                        $data['incExpBarChartData'] = ['income' => [], 'expense' => [], 'month' => []];
                    }
                    
                    \Log::info('About to render account dashboard view');
                    \Log::info('Data keys: ' . implode(', ', array_keys($data)));
                    
                    // TEMPORARY TEST - Uncomment to test if basic HTML works
                    // return response('<html><body><h1>Test - Dashboard Route Works!</h1></body></html>');
                    
                    return view('dashboard.account-dashboard', $data);
                } catch (\Exception $e) {
                    \Log::error('Account dashboard error: ' . $e->getMessage());
                    \Log::error('Error file: ' . $e->getFile() . ' Line: ' . $e->getLine());
                    \Log::error('Stack trace: ' . $e->getTraceAsString());

                    // If in debug mode, show the error
                    if(config('app.debug')) {
                        dd('Account Dashboard Error: ' . $e->getMessage(), $e->getTraceAsString());
                    }

                    // Otherwise redirect to HRM dashboard
